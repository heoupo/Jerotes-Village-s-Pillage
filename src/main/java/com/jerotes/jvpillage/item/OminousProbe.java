...
public class OminousProbe extends Item implements Vanishable {
	...
	public boolean isValidRepairItem(ItemStack itemStack, ItemStack itemStack2) {
		return itemStack2.is(JVPillageItems.ILLAGER_SIGNAL_LIGHT.get()) || super.isValidRepairItem(itemStack, itemStack2);
	}
		...
		if (Main.getTargetedEntity(player, 6) != null &&
			...
			if (!EntityFactionFind.isRaider(target)) {
				...
				if (level instanceof ServerLevel serverLevel) {
					for (int i = 0; i < 12; ++i) {
						serverLevel.sendParticles(JVPillageParticleTypes.ABACK.get(), target.getRandomX(0.5), target.getRandomY(), target.getRandomZ(0.5), 0, 0.0, 0.02, 0.0, 1);
					}
				}
				summonEntity(player, target, JVPillageEntityType.TELEPORTER.get(), 1, 6);
				summonEntity(player, target, JVPillageEntityType.EXECUTIONER.get(), 1, 6);
				summonEntity(player, target, JVPillageEntityType.BANNER_BEARER.get(), 1, 6);
				summonEntity(player, target, JVPillageEntityType.TRUMPETER.get(), 1, 6);
				summonEntity(player, target, JVPillageEntityType.EXPLORER.get(), Main.randomReach(player.getRandom(), 1, 3), 6);
				summonEntity(player, target, JVPillageEntityType.JAVELIN_THROWER.get(), Main.randomReach(player.getRandom(), 1, 3), 6);
