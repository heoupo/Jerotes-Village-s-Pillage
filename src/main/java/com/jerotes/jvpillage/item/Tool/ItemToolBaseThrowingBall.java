...
import com.jerotes.jvpillage.item.PotionThrowingBall;
...
public class ItemToolBaseThrowingBall extends Item implements JerotesItemThrowUse, CanGetOffHand {
    ...
    public void addEffects(LivingEntity livingEntity) {
        if (!livingEntity.level().isClientSide) {
            livingEntity.addEffect(new MobEffectInstance(JVPillageMobEffects.THROWING_STUBBORN_STONE.get(), 5, effectLevel - 1, false, false));
        }
    }
    ...
    public Projectile useJerotesThrowShoot(LivingEntity livingEntity, InteractionHand interactionHand) {
        if (livingEntity.getItemInHand(interactionHand).getItem() instanceof ItemToolBaseThrowingBall itemToolBaseThrowingBall) {
            if (!livingEntity.level().isClientSide()) {
                ...
                throwItem.addMaxBackCount = livingEntity.getItemInHand(interactionHand).getEnchantmentLevel(JVPillageEnchantments.ELASTICITY.get());
                //烟花
    ...
    public void appendHoverText(ItemStack itemStack, @Nullable Level level, List<Component> list, TooltipFlag tooltipFlag) {
        list.add(Component.translatable("item.jvpillage.throwing_ball", this.effectLevel, this.baseDamage, this.addDamage).withStyle(ChatFormatting.YELLOW));
