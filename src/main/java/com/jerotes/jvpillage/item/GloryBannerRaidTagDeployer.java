...
public class GloryBannerRaidTagDeployer extends Item {
	public GloryBannerRaidTagDeployer() {
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
		...
		if (level.dimension() == level.OVERWORLD && SpawnRules.BossSpawnFind(JVPillageEntityType.OMINOUS_BANNER_PROJECTION.get(), player)){
			if (level instanceof ServerLevel serverLevel) {
				BlockPos blockPos = Main.findSpawnPositionNearFillOnBlock(player, 6);
				OminousBannerProjectionEntity boss;

				if (blockPos != null) {
					boss = JVPillageEntityType.OMINOUS_BANNER_PROJECTION.get().spawn(serverLevel, blockPos, MobSpawnType.EVENT);
					if (boss != null) {
						boss.setLegend(true);
						boss.setChampion(true);
