...
import com.jerotes.jvpillage.entity.Monster.IllagerFaction.CyclonerEntity;
...
public class CyclonerRollLayer<T extends Entity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    private final ResourceLocation textureLocation;
    private final M model;

    public CyclonerRollLayer(RenderLayerParent<T, M> renderLayerParent, M m, ResourceLocation resourceLocation) {
    ...
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int n, T t, float f, float f2, float f3, float f4, float f5, float f6) {
        if (((CyclonerEntity)t).getSpellTick() <= 0 || !((CyclonerEntity)t).isAlive()) {
            return;
        }
