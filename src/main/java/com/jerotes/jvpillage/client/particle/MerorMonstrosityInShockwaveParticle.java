...
@OnlyIn(Dist.CLIENT)
public class MerorMonstrosityInShockwaveParticle extends TextureSheetParticle {
	private static final Vector3f TRANSFORM_VECTOR = new Vector3f(-1.0f, -1.0f, 0.0f);
	private final SpriteSet sprites;
	private float baseScale;
	private float distortionProgress;

	protected MerorMonstrosityInShockwaveParticle(ClientLevel level, double x, double y, double z,
                                                  SpriteSet sprites) {
		super(level, x, y, z);
	...
	public static class Provider implements ParticleProvider<SimpleParticleType> {
		...
		public Particle createParticle(SimpleParticleType type, ClientLevel level,
			...
			return new MerorMonstrosityInShockwaveParticle(level, x, y, z, this.sprites);
		}
	}
}