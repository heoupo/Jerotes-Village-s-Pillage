...
public class FollowFragranceGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        List<LivingEntity> list = this.follower.level()
                ...
                .stream()
                .filter(entity -> entity.hasEffect(JVPillageMobEffects.FRAGRANCE.get()))
                .toList();
        if (list.isEmpty()) {
            return false;
        }
        ...
        for (LivingEntity fragrance : list) {
            //
            int n = 0;
            if (fragrance.hasEffect(JVPillageMobEffects.FRAGRANCE.get())) {
               n = Objects.requireNonNull(fragrance.getEffect(JVPillageMobEffects.FRAGRANCE.get())).getAmplifier();
            }
            int effectLevel = n + 1;
