...
public class ArmorEvent {
		...
		if (event != null && event.getEntity() != null) {
			...
			if (!livingEntity.level().isClientSide()) {
				if (damageSource.is(DamageTypeTags.WITCH_RESISTANT_TO)) {
					if (livingEntity.getItemBySlot(EquipmentSlot.HEAD).getItem() == JVPillageItems.BIG_WITCHS_HAT.get()) {
	...
	public static boolean WarlockArmor(LivingHurtEvent event) {
		...
		boolean hasCassock = living.getItemBySlot(EquipmentSlot.CHEST).getItem() == JVPillageItems.WARLOCK_CASSOCK.get() || hasCurio(living, JVPillageItems.WARLOCK_CASSOCK.get());
		...
		if (hasCassock) {
			...
			living.removeEffect(JVPillageMobEffects.UNCLEAN_BODY.get());
		}
		...
		if (event != null && event.getEntity() != null) {
			...
			if (!livingEntity.level().isClientSide()) {
				...
				if (head.isPresent()) {
					...
					//水
					if (armorTrim.material().is(livingEntity.level().registryAccess().registryOrThrow(Registries.TRIM_MATERIAL).getHolder(ResourceKey.create(Registries.TRIM_MATERIAL, new ResourceLocation(JVPillage.MODID, "malignasaur_teeth"))).get().key())) {
	...
	public static void GiantMonsterHurt(LivingHurtEvent event) {
		if (event != null && event.getEntity() != null) {
			...
			if (damageSource == null || livingEntity == null)
				return;
			boolean helmet = livingEntity.getItemBySlot(EquipmentSlot.HEAD).getItem() == JVPillageItems.GIANT_MONSTER_HELMET.get()
					|| livingEntity.getItemBySlot(EquipmentSlot.HEAD).getItem() == JVPillageItems.GIANT_MONSTER_HORNED_HELMET.get();
			boolean chestplate = livingEntity.getItemBySlot(EquipmentSlot.CHEST).getItem() == JVPillageItems.GIANT_MONSTER_CHESTPLATE.get();
			boolean leggings = livingEntity.getItemBySlot(EquipmentSlot.LEGS).getItem() == JVPillageItems.GIANT_MONSTER_LEGGINGS.get();
			boolean boots = livingEntity.getItemBySlot(EquipmentSlot.FEET).getItem() == JVPillageItems.GIANT_MONSTER_BOOTS.get();
			if (!livingEntity.level().isClientSide()) {
	...
	public static void BlamerRobeTick(LivingEvent.LivingTickEvent event) {
		...
		if (livingEntity == null)
			return;
		if (livingEntity.getItemBySlot(EquipmentSlot.CHEST).getItem() == JVPillageItems.BLAMER_ROBE.get()) {
			if (!livingEntity.level().isClientSide()) {
			...
			if (livingEntity.getPersistentData().get("jerotes_cloaking_shift_key_down") != null
				...
				for (int i = 0; i < 7; ++i) {
					...
					livingEntity.level().addParticle(JVPillageParticleTypes.BLAMER_SOUL.get(), livingEntity.getRandomX(1.0), livingEntity.getRandomY() + 0.5, livingEntity.getRandomZ(1.0), d, d2, d3);
				}
			}
		}
	}
	...
	public static void SlaverySupervisorDeath(LivingDeathEvent event) {
		...
		if (entity instanceof Player player && livingEntity != null && damageSource != null) {
			...
			if (livingEntity instanceof AbstractIllager || livingEntity instanceof AbstractVillager) {
				boolean helmet = player.getItemBySlot(EquipmentSlot.HEAD).getItem() == JVPillageItems.SLAVERY_SUPERVISOR_HELMET.get();
				boolean chestplate = player.getItemBySlot(EquipmentSlot.CHEST).getItem() == JVPillageItems.SLAVERY_SUPERVISOR_CHESTPLATE.get();
				boolean leggings = player.getItemBySlot(EquipmentSlot.LEGS).getItem() == JVPillageItems.SLAVERY_SUPERVISOR_LEGGINGS.get();
				boolean boots = player.getItemBySlot(EquipmentSlot.FEET).getItem() == JVPillageItems.SLAVERY_SUPERVISOR_BOOTS.get();
	...
	public static void BuffAboutAddEffect(MobEffectEvent event) {
		...
		if (mobEffectInstance == null)
			return;
		//神汉教袍
		boolean hasCassock = livingEntity.getItemBySlot(EquipmentSlot.CHEST).getItem() == JVPillageItems.WARLOCK_CASSOCK.get() || hasCurio(livingEntity, JVPillageItems.WARLOCK_CASSOCK.get());
		if (hasCassock) {
			if (mobEffectInstance.getEffect() == MobEffects.CONFUSION ||
					mobEffectInstance.getEffect() == JVPillageMobEffects.UNCLEAN_BODY.get())
