package com.jerotes.jvpillage.alchemy;
...
public class MaterialEvent {
	...
	public static void Material(JerotesAlchemyMaterialEffectEvent event) {
		...
		if (itemStack == null)
			return;
		//明路脆瓜
		if (itemStack.is(JVPillageItems.BRIGHT_MELON.get())) {
		...
		//金明路脆瓜
		if (itemStack.is(JVPillageItems.GOLDEN_BRIGHT_MELON.get())) {
			event.setEffectCount(3);
		...
		//二轮龙葵
		if (itemStack.is(JVPillageItems.SECOND_ROUND_NIGRUM.get())) {
			event.setEffectCount(3);
		...
		//冰岩
		if (itemStack.is(JVPillageItems.ICE_ROCK.get())) {
			event.setEffectCount(3);
		...
		//鳞熊毛 巨怪毛 巨怪角
		if (itemStack.is(JVPillageItems.GIANT_MONSTER_HAIR.get())  || itemStack.is(JVPillageItems.GIANT_MONSTER_HORN.get())) {
			event.setEffectCount(3);
		...
		//恶鳞鳞片 恶鳞牙齿
		if (itemStack.is(JVPillageItems.MALIGNASAUR_TEETH.get())) {
			event.setEffectCount(3);
