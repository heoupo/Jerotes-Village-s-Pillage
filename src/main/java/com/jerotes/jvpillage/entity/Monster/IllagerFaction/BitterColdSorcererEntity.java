...
public class BitterColdSorcererEntity extends SpellIllagerEntity implements SpellUseEntity , BannerChampionEntity {
    ...
    public List<SpellTypeInterface> SelfMainSpellList() {
        ...
        if (!(this.getTarget() != null && this.distanceTo(this.getTarget()) > 8)) {
            //技能列表-苦寒冰霜
            spellList.add(OtherSpellType.JVPILLAGE_BITTER_COLD_FROSTBITE);
        }
        else {
            //技能列表-苦寒冰锥
    ...
    protected SoundEvent getAmbientSound() {
        return JVPillageSoundEvents.BITTER_COLD_SORCERER_AMBIENT;
    }
    @Override
    protected SoundEvent getDeathSound() {
        return JVPillageSoundEvents.BITTER_COLD_SORCERER_DEATH;
    }
    @Override
    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return JVPillageSoundEvents.BITTER_COLD_SORCERER_HURT;
    }
    @Override
    public SoundEvent getCelebrateSound() {
        return JVPillageSoundEvents.BITTER_COLD_SORCERER_CHEER;
    }
    @Override
    protected SoundEvent getCastingSoundEvent() {
        return JVPillageSoundEvents.BITTER_COLD_SORCERER_SHOOT_1;
    }
    @Override
    ...
    protected Component getTypeName() {
        if (this.isChampion())
            return Component.translatable("entity.jvpillage.bitter_cold_sorcerer.champion");
    ...
    public void aiStep() {
        ...
        //苦寒祭坛
            ...
            if (this.getRandom().nextInt(6 * 20) == 1 && this.getSpellOneTick() <= 0 && this.getSpellTick() <= 0 && this.isAggressive()) {
                ...
                if (!this.isSilent()) {
                    this.level().playSound(null, this.getX(), this.getY(), this.getZ(), JVPillageSoundEvents.BITTER_COLD_SORCERER_ALTAR, this.getSoundSource(), 5.0f, 0.8f + this.getRandom().nextFloat() * 0.4f);
                }
            }
        }
        //站立动画
