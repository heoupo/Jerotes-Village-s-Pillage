...
public class BlasterEntity extends MeleeIllagerEntity implements AlwaysShowArmIllagerEntity, RangedAttackMob, UseThrowEntity {
    ...
    public void performRangedAttack(LivingEntity livingEntity, float f) {
        ...
        if (!this.isSilent()) {
            this.playSound(JVPillageSoundEvents.BLASTER_THROW, 1.0f, 1.0f / (this.getRandom().nextFloat() * 0.4f + 0.8f));
        }
    ...
    protected SoundEvent getAmbientSound() {
        return JVPillageSoundEvents.BLASTER_AMBIENT;
    }
    @Override
    protected SoundEvent getDeathSound() {
        return JVPillageSoundEvents.BLASTER_DEATH;
    }
    @Override
    protected SoundEvent getHurtSound(DamageSource damageSource) {
        ...
        return JVPillageSoundEvents.BLASTER_HURT;
    }
    @Override
    public SoundEvent getCelebrateSound() {
        return JVPillageSoundEvents.BLASTER_CHEER;
    }
    @Override
    public void applyRaidBuffs(int n, boolean bl) {
        boolean bl2;
        ItemStack itemStack = new ItemStack(JVPillageItems.OMINOUS_CUTLASS.get());
    ...
    public boolean doHurtTarget(Entity entity) {
        ...
        if (bl) {
            if (!this.isSilent()) {
                this.playSound(JVPillageSoundEvents.BLASTER_ATTACK, 1.0f, 1.0f);
            }
        }
        return bl;
    }
    ...
    public ItemStack createSpawnWeapon(float weaponRandom) {
        return new ItemStack(JVPillageItems.OMINOUS_CUTLASS.get());
    }
    @Override
    public ItemStack createSpawnOffhand(float offhandRandom) {
        return new ItemStack(JVPillageItems.OMINOUS_BOMB.get());
    }
}
