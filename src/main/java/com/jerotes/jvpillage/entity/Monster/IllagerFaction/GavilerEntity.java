...
public class GavilerEntity extends SpellIllagerEntity implements RangedAttackMob, SpellUseEntity, EliteEntity, BannerChampionEntity {
    ...
    protected SoundEvent getAmbientSound() {
        return JVPillageSoundEvents.GAVILER_AMBIENT;
    }
    @Override
    protected SoundEvent getDeathSound() {
        return JVPillageSoundEvents.GAVILER_DEATH;
    }
    @Override
    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return JVPillageSoundEvents.GAVILER_HURT;
    }
    @Override
    public SoundEvent getCelebrateSound() {
        return JVPillageSoundEvents.GAVILER_CHEER;
    }
    @Override
    protected SoundEvent getCastingSoundEvent() {
        return JVPillageSoundEvents.GAVILER_SHOOT;
    }
    @Override
    ...
    protected Component getTypeName() {
        if (this.isChampion())
            return Component.translatable("entity.jvpillage.gaviler.champion");
    ...
    public void aiStep() {
        ...
        if (!this.onGround()) {
            for (int i = 0; i < 3; ++i) {
                if (this.level().isClientSide) {
                    this.level().addParticle(JVPillageParticleTypes.PUSH_FORCE.get(), this.getRandomX(1.5), this.getY(), this.getRandomZ(1.5), 0.0, 0.0, 0.0);
                }
            }
        }
        //上升
