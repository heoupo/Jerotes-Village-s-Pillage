...
public class CarvedIronGolemMeleeAttackGoal extends Goal {
    protected final CarvedIronGolemEntity mob;
    ...
    public CarvedIronGolemMeleeAttackGoal(CarvedIronGolemEntity pathfinderMob, double d, boolean bl) {
    ...
    public boolean canUse() {
        if (this.mob.getNetheriteTick() <= 0) {
            return false;
        }
        long l = this.mob.level().getGameTime();
        if (l - this.lastCanUseCheck < 0L) {
            return false;
        }
    ...
    protected boolean canPerformAttack(LivingEntity livingEntity) {
        return (this.isTimeToAttack() || this.mob.getNetheriteTick() > 0) && this.mob.isWithinMeleeAttackRange(livingEntity) && this.mob.getSensing().hasLineOfSight(livingEntity);
    }
    ...
    protected int getAttackInterval() {
        return this.adjustedTickDelay(0);
    }
}
